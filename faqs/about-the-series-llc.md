...
This smart contract is created by sending a given amount of DAI, currently 5, to`otocorp.eth`. 

This `otocorp.eth`contract contains the  [Master LLC Operating Agreement ](../operating-agreements/otocorp-master-llc-operating-agreement.md)which derives its authority to establish Series LLCs directly from the Act. 
...
After the DAI is received, the Series LLC Operating Agremeent is signed with the public wallet address of the sender, which legally becomes the LLCs first Member.
...
In a traditional context, the Master is typcially the "sponsor" of a common project that requires individual LLCs for each asset held, e.g. individual properties financed by the same sponsor who is typically Member in each of the LLCs.
...
Such structure could not work if there is any risk of cross-contamination, and this key operating principle is expresssly articulated in the Agreement.
...
Categorically yes. Your LLCs is entirely indepedent from other Series LLCs: it has its own Members, Managers, assets and liabilities, just like a normal LLC.  
...
* You do not have to pay the Delaware Annual Franchise tax since only the Master, OtoCo LLC, pays the tax, not the individual Series.
* Your LLC is automatically given a Delaware address.  For a small extra fee you can have Otomomos provide a business address in all major US States where you can receive physical mail and have it scanned to you.
...
* Finaly, the LLC can also be entirely anonymous if setup via Otonomos: as long as the address you use to spin up your LLC is not whitelisted, your identity will not be publicly linked to the ownership of the LLC \(see below "How about indentity"\). 
...
5 dollars -  the price of a cup of coffee. 
...
All you have to do is go to [https://otoco.io](https://otoco.io) and send 5 DAI to `otocorp.eth` to instantly activate your LLC.
...
2. If the name is available, just connect the Metamask wallet you want to use to send the 5 DAI. Note that this address will be the address that controls the smart contract that is created for your LLC.
3. After confirming the address, you send 5 DAI and your company will typically activate within 5 seconds.  The address for your LLC smart contract will show and you will receive an executed copy of its Operating Agreement by email.
...
The chosen name will appear on the cover page of the LLC's Opertating Agreement. The business of the Company may be conducted under that name or under any other name that the Manager may determine. 
...
E.g. for Acme LLC, conventional namig would typically be: `Acme LLC, a Series of OtoCo LLC` however there is nothing in the Act that prohibits the use of the Series LLC by its indivdual name, `Acme LLC.`

## How does this work at the blockchain level?

There's two smart contracts at work and a fair bit of legal engineering.

Simplified, receipt of the 5 DAI to the Master smart contract for OtoCo prompts the Master to create a new smart contract for every individual Series LLC.

Your Series LLC smart contract is controlled by the wallet you used to send the DAI.  This means that the first Member of the LLC is effectively a wallet address, since it is that address that signed the new LLCs Operating Agreement. 

The smart contracts are fully open source and can be found here: \[Link\].
...
1. the final Distribution of the net assets of the Company to the Members or a Liquidating Vehicle in case of an involutary winding-up;
...
The Registered Agent, i.e. Otonomos LLC, is the LLC's registered agent for service of process.
...
In addition to offering OtoCo for C-Corps and in more jurisdictions, our technology roadmap includes adding layer after layer of blockchain solutions for all painpoints in the LLC, from tokenizing and transferring Member interests to a genuine online crypto banking solution for your LLC. 
